...
left join productos as c on b.idunicop=c.idunicop left join catusuarios as d on a.idunicou=d.idunicou left join sucursal as e on a.idunicosr=e.idunicos where a.fecha>='$fechai' and a.fecha<='$fechaf' and a.tiposal=1 and a.idunicoe='$idunicoe' and a.idunicos='$idunicos'";
$consulta=$consulta.$cmodelo.$ctalla."order by a.idunicosal";
$recordset = mysqli_query($link,$consulta);
$cuentaregistros=0;
...
$html1='<table border="0" align="center">          
 ...
 <tr>
	...
	<td class="tds1" width="60%"align="center"><font style="font-size:40px" face="Arial, Helvetica, sans-serif" ><strong><br>SISTEMA DE PUNTO DE VENTA <BR> '.$nombreempresa.'<br>Reporte de Traspaso</strong></font>
   </td>
...
/////////////////////////carga de datos a mostrar principales*********************************************
     $consulta="select d.nombre,a.tiposal,a.ticket,a.idunicotras,a.baja,c.codigo,c.nombrep,a.fecha,b.cantidad,b.pmostrador,b.descuento,b.iva,b.pfinal,e.nombre as sucur from mtosalidas as a left join detsalidas as b on a.idunicosal=b.idunicosal 
left join productos as c on b.idunicop=c.idunicop left join catusuarios as d on a.idunicou=d.idunicou left join sucursal as e on a.idunicosr=e.idunicos where a.fecha>='$fechai' and a.fecha<='$fechaf' and a.tiposal=1 and a.idunicoe='$idunicoe' and a.idunicos='$idunicos' ";
	$consulta=$consulta.$cmodelo.$ctalla."order by a.idunicosal";
    $recordset = mysqli_query($link,$consulta);
	$total=0;
    ...
    while($registro = mysqli_fetch_array($recordset)){
		...
		$numero=$numero.' -->Traspaso a '.$registro["sucur"];								
